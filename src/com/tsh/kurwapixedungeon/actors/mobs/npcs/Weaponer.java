 ...
 */
package com.tsh.kurwapixedungeon.actors.mobs.npcs;
...
import com.tsh.kurwapixedungeon.Journal;
...
import com.tsh.kurwapixedungeon.actors.mobs.CursePersonification;
import com.tsh.kurwapixedungeon.actors.mobs.FetidRat;
...
import com.tsh.kurwapixedungeon.items.quest.Ammo;
import com.tsh.kurwapixedungeon.items.quest.DriedRose;
import com.tsh.kurwapixedungeon.items.quest.RatSkull;
...
import com.tsh.kurwapixedungeon.sprites.GhostSprite;
import com.tsh.kurwapixedungeon.utils.Utils;
import com.tsh.kurwapixedungeon.windows.WndQuest;
import com.tsh.kurwapixedungeon.windows.WndSadGhost;
import com.tsh.kurwapixedungeon.windows.WndWeap;
...
public class Weaponer extends NPC {
	...
	public String description() {
		return 
			"A guy who sell guns.His buisness out if order cause" +
			"all of guns were sold.Maybe i can help him...";
	}
	...
	public static class Quest {
		...
		enum Type {
			ILLEGAL( null ), ROSE( roseQuest ), RAT( ratQuest ), CURSE( curseQuest ), AMMO (AmmoQuest);
		...
		private static final String NODE		= "Weaponer";
		...
		public static void complete() {
			...
			Journal.remove( Journal.Feature.WEAP );
		}
	}

	abstract public static class QuestHandler {
		
		abstract public void interact(  Weaponer ghost ); 
		
		protected void relocate( Weaponer ghost ) {
			int newPos = -1;
			for (int i=0; i < 10; i++) {
	...
	private static final QuestHandler roseQuest = new QuestHandler() {
		private static final String TXT_ROSE1	=
			"Halo adventur!Nies to met yu... " +
			"I wanna gut some ammo, yknow? _7.92 ammo pack_... " +
			"It's very important to me... Find it pleas!";
		
		private static final String TXT_ROSE2	=
			"Please... Help me... _Find 7.92 ammo pack_...";
		...
		public void interact( Weaponer ghost ) {
			if (Quest.given) {

				Item item = Dungeon.hero.belongings.getItem( DriedRose.class );	
				if (item != null) {
					GameScene.show( new WndWeap( ghost, item, TXT_ROSE3 ) );
				} else {
				...
				Journal.add( Journal.Feature.WEAP );
			}
		}
	};
	
	private static final QuestHandler AmmoQuest = new QuestHandler() {
		private static final String TXT_RAT1	=
			"Yo guy, nice to meet ya.There is a little big sad problem: i need ammo.Full or not, i'll take any.Maybe this _AWP_ will help you with that?";
			
		private static final String TXT_RAT2	=
			"I waitin' for ammo, hommie.";
		
		private static final String TXT_RAT3	= 
			"Fucking Jesus!Ya found it?Thanks!Take this and get fuck outa here.";
		
		public void interact( Weaponer ghost ) {
			if (Quest.given) {

				Item item = Dungeon.hero.belongings.getItem( Ammo.class );	
				if (item != null) {
					GameScene.show( new WndWeap( ghost, item, TXT_RAT3 ) );
				} else {
	...
	private static final QuestHandler curseQuest = new QuestHandler() {
		...
		private static final String TXT_CURSE2 =
			"Thank you... " +
			"Please take... one of these _items_, maybe they " +
			"will be useful to you in your journey...";
		
		private static final String TXT_YES	= "Yes, I will do it for you";
		private static final String TXT_NO	= "No, I can't help you";
		
		public void interact( final Weaponer ghost ) {
			if (Quest.given) {

				GameScene.show( new WndWeap( ghost, null, Utils.format( TXT_CURSE2, Dungeon.hero.className() ) ) );
