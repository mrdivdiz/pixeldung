...
import com.tsh.kurwapixedungeon.items.ItemStatusHandler;
import com.tsh.kurwapixedungeon.items.KindOfWeapon;
import com.tsh.kurwapixedungeon.items.bags.Bag;
import com.tsh.kurwapixedungeon.items.rings.RingOfPower.Power;
...
public abstract class Wand extends KindOfWeapon {
	...
	public void execute( Hero hero, String action ) {
		if(Hero.mana >= 2 + 2 * (maxCharges-2)){
			Hero.mana -= 2 + 2 * (maxCharges-2);
		if (action.equals( AC_ZAP )) {
			...
			GLog.n("You have no mana!Shoot require " + (2 * (maxCharges-2)) + " MP!");
		}
	}
