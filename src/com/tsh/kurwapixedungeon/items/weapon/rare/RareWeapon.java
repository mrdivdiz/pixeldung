...
public class RareWeapon extends Weapon {
	...
	public RareWeapon( int tier, float acu, float dly ) {
		super();
	...
	public String info() {
		...
		String quality = lvl != 0 ? 
			...
			"";
		info.append( p );
		info.append( "This _" + name + "_ is " + Utils.indefinite( quality ) );
		info.append( " _rare_ weapon.Better to keep it in your pack. " );
		
		if (levelKnown) {
			int min = min();
			int max = max();
			info.append( "Its _average damage is " + (min + (max - min) / 2) + "_ points per hit. " );
			
		} else {
			int min = min0();
			int max = max0();
			info.append( 
				"Its typical average _damage is " + (min + (max - min) / 2) + "_ points per hit " +
				"and usually it _requires " + typicalSTR() + " points of strength._ " );
			info.append( 
					p);
			info.append( 
					"Its _tier is " + tier + "_ .It _accuracy is " +  ACU + "_ .It speed is _" + DLY + "_");
			if (typicalSTR() > Dungeon.hero.STR()) {
				info.append( "Probably this weapon is _too heavy_ for you. " );
			}
		}
		
		if (DLY != 1f) {
			info.append( "This is a rather _unstable_ and " + (DLY < 1f ? "_fast_" : "_slow_") );
			if (ACU != 1f) {
				if ((ACU > 1f) == (DLY < 1f)) {
					info.append( " and ");
				} else {
				...
				info.append( ACU > 1f ? "_accurate_" : "_inaccurate_" );
			}
			info.append( " weapon. ");
		} else if (ACU != 1f) {
			info.append( "This is a rather " + (ACU > 1f ? "_accurate_" : "_inaccurate_") + " weapon. " );
		}
		switch (imbue) {
		case SPEED:
