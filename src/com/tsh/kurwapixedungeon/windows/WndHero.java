...
public class WndHero extends WndTabbed {
	...
	//public int hungo = Hunger.hungerity;
	...
	public WndHero() {
		...
		for (Tab tab : tabs) {
			tab.setSize( TAB_WIDTH, tabHeight() - 5 );
		}
		
		resize( WIDTH, (int)Math.max( stats.height(), buffs.height() ) + 20);
	...
	private class StatsTab extends Group {
		...
		public StatsTab() {
			...
			RedButton btnSkills = new RedButton( "Skills..." ) {
				@Override
				protected void onClick() {
					hide();
			...
			btnSkills.setRect( 30, 95, btnCatalogus.reqWidth() + 2, btnCatalogus.reqHeight() + 2 );
			add( btnSkills );
			...
			statSlot( TXT_MANA, Hero.mana + "/" + Hero.manat );
			statSlot( "Attack", Hero.attackSkill);
			statSlot( "Defence", Hero.defenseSkill);
			...
			pos += GAP;
			statSlot( "Hunger", (int)Hunger.level);
		}
