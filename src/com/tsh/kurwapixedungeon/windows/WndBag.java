...
public class WndBag extends WndTabbed {
	...
	public WndBag( Bag bag, Listener listener, Mode mode, String title ) {
		...
		nCols = KurwaDungeon.landscape() ? COLS_L : COLS_P;
		...
		Bag[] bags = {
			...
			stuff.getItem( TrashBag.class ),
			stuff.getItem( Keyring.class ),
			stuff.getItem( Ringbag.class ),
		    stuff.getItem(EatBag.class)};
		
		for (Bag b : bags) {
			if (b != null) {
				BagTab tab = new BagTab( b );
				tab.setSize( 15, tabHeight() );
				add( tab );
			...
			if (item != null) {
				...
				if (item.name() == null) {
					...
					enable( 
						...
						mode == Mode.CRAFTABLE && item.isCraftable() || 
