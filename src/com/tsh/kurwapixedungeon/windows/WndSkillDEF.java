...
public class WndSkillDEF extends Window {
	...
	public WndSkillDEF() {
		...
		text1.text( "Improve your defence skill, gain one protection point per level", width);
		text1.setPos( 0, 0 );
		add( text1 );
		
		GuideboardItem GUIDE_ARMOR3 = new GuideboardItem( "Improve", 11 ) {
			@Override
			protected void onClick() {
				if(Hero.sp >= 1 && Hero.defskill <= 9){
					
					Hero.defenseSkill++;
					Hero.defskill++;
					Hero.sp--;
					GLog.i("Upgraded!Now it is" + Hero.defskill + "/10");
					}
					if(Hero.defskill >= 10 && Hero.sp > 0){
					Hero.defskill = 10;
					hide();
		...
		add(GUIDE_ARMOR3);
		GUIDE_ARMOR3.setPos(50 - GUIDE_ARMOR3.width(), 30);
