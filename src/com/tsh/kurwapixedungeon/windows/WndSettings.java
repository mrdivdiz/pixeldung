...
import com.tsh.kurwapixedungeon.scenes.WelcomeScene;
import com.tsh.kurwapixedungeon.themes.Theme;
import com.tsh.kurwapixedungeon.themes.ThemeLoader;
import com.tsh.kurwapixedungeon.ui.CheckBox;
...
public class WndSettings extends Window {
	...
	public static final String TXT_SND		= "Themes";
	...
	public WndSettings( boolean inGame ) {
		...
		if (inGame) {
			...
			CheckBox btnScaleUp = new CheckBox( TXT_SCALE_UP ) {
				...
				protected void onClick() {
					super.onClick();
					KurwaDungeon.scaleUp( checked() );
				}
			};
			btnScaleUp.setRect( 0, 0, WIDTH, BTN_HEIGHT );
			btnScaleUp.checked( KurwaDungeon.scaleUp() );
			add( btnScaleUp );
			...
			btnImmersive = new CheckBox( TXT_IMMERSIVE ) {
				...
				protected void onClick() {
					super.onClick();
					KurwaDungeon.immerse( checked() );
				}
			};
			btnImmersive.setRect( 0, btnScaleUp.bottom() + GAP, WIDTH, BTN_HEIGHT );
			btnImmersive.checked( KurwaDungeon.immersed() );
		...
		CheckBox btnMusic = new CheckBox( TXT_MUSIC ) {
			...
			protected void onClick() {
				super.onClick();
				KurwaDungeon.music( checked() );
				
			}
		};
	//	Button btntheme = new Button( TXT_SND ) {
			//@Override
			//protected void onClick() {
			//	super.onClick();
	...
	//	};
		...
		btnMusic.checked( KurwaDungeon.music() );
		add( btnMusic );
		...
		CheckBox btnSound = new CheckBox( TXT_SOUND ) {
			...
			protected void onClick() {
				super.onClick();
				KurwaDungeon.soundFx( checked() );
				Sample.INSTANCE.play( Assets.SND_CLICK );
			}
		};
		btnSound.setRect( 0, btnMusic.bottom() + GAP, WIDTH, BTN_HEIGHT );
		btnSound.checked( KurwaDungeon.soundFx() );
		add( btnSound );
		...
		if (inGame) {
			...
			CheckBox btnBrightness = new CheckBox( TXT_BRIGHTNESS ) {
				...
				protected void onClick() {
					super.onClick();
					KurwaDungeon.brightness( checked() );
				}
			};
			btnBrightness.setRect( 0, btnSound.bottom() + GAP, WIDTH, BTN_HEIGHT );
			btnBrightness.checked( KurwaDungeon.brightness() );
			add( btnBrightness );
			...
			RedButton btnOrientation = new RedButton( orientationText() ) {
				@Override
				protected void onClick() {
					KurwaDungeon.landscape( !KurwaDungeon.landscape() );
				}
			};
			btnOrientation.setRect( 0, btnSound.bottom() + GAP, WIDTH, BTN_HEIGHT );
			add( btnOrientation );
			CheckBox btnThem = new CheckBox( TXT_SND ) {
				@Override
				protected void onClick() {
					KurwaDungeon.switchNoFade( ThemeScene.class );
			...
			btnThem.setRect( 0, btnOrientation.bottom() + GAP, WIDTH, BTN_HEIGHT );
			btnThem.checked( KurwaDungeon.theme() );
			add( btnThem );
			
			resize( WIDTH, (int)btnThem.bottom() );
	...
	private void zoom( float value ) {
		...
		KurwaDungeon.zoom( (int)(value - PixelScene.defaultZoom) );
	...
	private String orientationText() {
		return KurwaDungeon.landscape() ? TXT_SWITCH_PORT : TXT_SWITCH_LAND;
	}
}
